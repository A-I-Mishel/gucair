...
/**
 * Recalculate all rankings from approved universities with non-null scores.
 * Updates /stats/singleton and /rankings/{region}.
 */
  ...
  try {
    ...
    for (const r of REGIONS) {
      ...
      regionAvgs[r] = {
        ...
        avgScore: avg(
          inRegion.filter((u) => typeof u.score === "number").map((u) => u.score as number)
        ),
      };
    }
    ...
    return NextResponse.json({ ok: true, ranked: scored.length, approved: unis.length });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : "Recalculate failed" }, { status: 500 });
  }
}
